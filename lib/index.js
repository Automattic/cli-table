...
Table.prototype.toString = function (){
  ...
  // NOTE: Columns which have required width below minColumnWidth will be rendered with their required width.  
  var minColumnWidth = options.minColumnWidth;
  ...
  if (!colWidths.length){
    ...
    all_rows.forEach(function(cells){
      ...
      if (typeof cells === 'object' && cells.length) {
        extractColumnWidths.call(scope,cells);
      // vertical (objects)
      } else {
        var header_cell = Object.keys(cells)[0]
          , value_cell = cells[header_cell];

        colWidths[0] = Math.max(colWidths[0] || 0, get_width.call(scope,header_cell) || 0);

        // cross (objects w/ array values)
        if (typeof value_cell === 'object' && value_cell.length) {
          extractColumnWidths.call(scope,value_cell, 1);
        } else {
          colWidths[1] = Math.max(colWidths[1] || 0, get_width.call(scope,value_cell) || 0);
        }
      }
    });

    var calc_required_width = function(){
      var sum = 0;
    ...
    var required_width = calc_required_width();
    if(terminal_width <= required_width) {
      ...
      colWidths = colWidths.map(function(col, index){
        ...
        if(calculated_value >= minColumnWidth) {
    ...
    var calc_req_width = calc_required_width();
    if(calc_req_width > terminal_width) {
      console.warn("Calculated width "+ calc_req_width +" is above terminal width: " + terminal_width + ". You can try using smaller value for minColumnWidth. Current value is: " + minColumnWidth);
    }
  };
  ...
  function extractColumnWidths(arr, offset) {
    ...
    arr.forEach(function(cell, i){
      colWidths[i + offset] = Math.max(colWidths[i + offset] || 0, get_width.call(scope,cell) || 0);
    });
  };

  function get_width(obj) {
    if(typeof obj !== 'undefined' && obj !== null){
      var width = typeof obj == 'object' && obj.width != undefined
         ? obj.width
         : ((typeof obj == 'object' ? utils.strlen(obj.text) : utils.strlen(obj)) + (style['padding-left'] || 0) + (style['padding-right'] || 0))
      return Math.max(width,this.options.minColumnWidth);
    }
    else{
  ...
  */
  function beautify_string(content, wrap_line_length) {
    ...
    if (content) {
      ...
      var remaining = "";
      contentArr.map(function(line){
        if (line.length > wrap_value) {
          var row = _.take(line.split(""), wrap_value);

          // check if the symbol after wrap_value is delimiter;
          if (isDelimiter(line[wrap_value])) {
            wrap_value = get_index_after_code(line, wrap_value);
            current_line = line.substring(0, wrap_value).trim();
            remaining = line.substring(wrap_value).trim();
          } else {
            ...
            index = get_index_after_code(line, index);
            current_line = line.substring(0, index).trim();
            remaining = line.substring(index).trim();
          }
  ...
  */
  ...
  function generateRow (items, style) {
    ...
    items.forEach(function (item, i) {
      var contents = beautify_string((item === null || typeof item === 'undefined') ? '' : item.toString(), this.options.colWidths[i])
        .split("\n")
